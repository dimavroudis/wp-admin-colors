...
export class GeneratorService {
	...
	private css: string;
	private php: string;
	private lastDefaultColorIndex: number = 0;

	progress: EventEmitter<{ status: string; message: string; done: boolean }> =
		new EventEmitter();
	...
	rotateColors(): Observable<Color[]> {
		...
		return rotating.pipe(
			map((index) => {
				this.lastDefaultColorIndex = index % lenght;
				return (this.colors = this.colorsList[this.lastDefaultColorIndex]);
			})
		);
	}

	getNextColorScheme() {
		let nextIndex = this.lastDefaultColorIndex + 1;
		this.lastDefaultColorIndex =
			nextIndex >= this.colorsList.length ? 0 : nextIndex;
		return (this.colors = this.colorsList[this.lastDefaultColorIndex]);
	}
	...
	private generatePHP(): Promise<void> {
		return fetch("assets/generator/functions.php.txt")
			...
			.then((results) => {
				return results
					...
					.replace(
						...
						this.getColor("notification-color")
					)
			...
			.then((php) => {
				this.php = php;
				this.progress.emit({
					status: "generated:php",
					message: "PHP generated.",
					done: !!this.php && !!this.css,
				});
			})
			.catch(() => {
				this.progress.emit({
					status: "failed",
					message: "Failed to load functions.php.txt",
					done: true,
				});
			});
	}
	...
	private mergeSass(): Promise<string | Error> {
		return fetch("assets/generator/wp_admin.scss")
			.then((template) => template.text())
			.then((results) => {
	...
	private convertSASStoCSS(sass): void {
		try {
			...
			this.progress.emit({
				status: "failed",
				message: "Failed to generate CSS from SASS.",
				done: true,
			});
		}
	}
